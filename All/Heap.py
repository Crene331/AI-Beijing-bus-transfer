...
def initHeap(minheap):
	...
	for i in range(int(haslastleaf),-1,-1):
		holdHeap(minheap,i)
...
def decHeap(minheap,station_name,newcost,parent):
	...
	while parentPos >= 0:
		if minheap.heap[int(targetPos)].cost >= minheap.heap[int(parentPos)].cost:
			break
		minheap.heap[int(targetPos)],minheap.heap[int(parentPos)]=\
		minheap.heap[int(parentPos)],minheap.heap[int(targetPos)]
		targetPos = parentPos
		parentPos = (targetPos-1)/2